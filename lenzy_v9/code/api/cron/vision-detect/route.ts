/**
 * /api/cron/vision-detect/route.ts — Next.js App Router cron route handler
 *
 * Purpose: Vercel cron entry point for the Gemini Vision eyewear detection
 *          step. Verifies CRON_SECRET header, runs runVisionDetect() in
 *          batches of 50 rows (default), returns JSON stats. Wrapped with
 ...
 * Vercel cron path: /api/cron/vision-detect
 *
 * Example invocation (local test):
 *   curl -X POST http://localhost:3000/api/cron/vision-detect \
...
import { runVisionDetect } from '@/lib/ingestion/vision-detect';
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  if (!secret || secret !== process.env.CRON_SECRET) {
    logger.warn({ path: '/api/cron/vision-detect' }, 'cron: unauthorized request');
    return NextResponse.json({ error: 'Unauthorized' }, { status: 401 });
  }

  const step = 'vision-detect';
  ...
  try {
    stats = await runVisionDetect(DEFAULT_BATCH_SIZE);
