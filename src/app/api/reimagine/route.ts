...
/**
 * Persist a reimagined image to Vercel Blob. Replicate URLs expire
 * in 24-48 hours, so we fetch the generated bytes and upload them
 * to Blob for permanent storage.
 *
 * Returns the permanent blob URL, or null if upload fails (in which
 * case we keep the original Replicate URL — better than nothing).
 */
async function persistToBlob(imageUrl: string, brandHandle: string): Promise<{ blobUrl: string | null; bytes: number }> {
  if (!BLOB_TOKEN) return { blobUrl: null, bytes: 0 };
  if (!imageUrl || imageUrl.startsWith('data:')) return { blobUrl: null, bytes: 0 };
  ...
  try {
    ...
    if (!res.ok) return { blobUrl: null, bytes: 0 };
    const buf = await res.arrayBuffer();
    if (buf.byteLength < 1024 || buf.byteLength > 15 * 1024 * 1024) {
      return { blobUrl: null, bytes: buf.byteLength };
    }
    const contentType = res.headers.get('content-type') || 'image/jpeg';
    const ext = contentType.includes('png') ? 'png' : contentType.includes('webp') ? 'webp' : 'jpg';
    const timestamp = Date.now();
    const random = Math.random().toString(36).slice(2, 10);
    const path = `reimagine/${brandHandle || 'brand'}/${timestamp}-${random}.${ext}`;
    ...
    return { blobUrl: url, bytes: buf.byteLength };
  } catch (e) {
    console.warn('[persistToBlob]', e instanceof Error ? e.message : e);
    return { blobUrl: null, bytes: 0 };
  }
}

/**
 * Log a reimagine generation into brand_content with parent_id
 * pointing to the source post. Persists the output image to
 * Vercel Blob for permanent storage before inserting.
 */
async function logReimagine(opts: {
  brandName: string;
  imageUrl: string;           // source image URL (what the user reimagined)
  generatedImages: Array<{ url: string; model: string; type: string }>;
  creativeBrief: string;
  imagePrompt: string;
  productImageUrl: string | null;
}): Promise<Array<{ model: string; original: string; blob: string | null; bytes: number; id: number | null }>> {
  const results: Array<{ model: string; original: string; blob: string | null; bytes: number; id: number | null }> = [];
  try {
    ...
    const { data: brand } = await client
      ...
      .limit(1)
      .maybeSingle();
    const brandId = brand ? (brand as { id: number }).id : null;
    const brandHandle = brand ? (brand as { handle: string }).handle : opts.brandName.toLowerCase();

    // Find the source post (parent) in brand_content
    ...
    // For each generated image: persist to Blob, then insert a row
    for (const g of opts.generatedImages) {
      if (!g.url) continue;

      // Skip Pollinations fallback URLs — they're generated on-the-fly per request
      // and the URL itself IS the "image" (parameterized). Still insert the row for
      // traceability but don't try to download.
      const shouldPersist = !g.url.includes('image.pollinations.ai');
      const { blobUrl, bytes } = shouldPersist
        ? await persistToBlob(g.url, brandHandle)
        : { blobUrl: null, bytes: 0 };
      ...
      const { data: inserted } = await client
        ...
        .insert({
          ...
          image_url: g.url,                      // original (Replicate URL — may expire)
          blob_url: blobUrl,                     // permanent Vercel Blob copy
          thumbnail_url: blobUrl || g.url,
          url: opts.productImageUrl || null,
          source: 'reimagine',
          source_ref: `${g.model}:${Date.now()}`,
          data: {
            model: g.model,
            kind: g.type,
            image_prompt: opts.imagePrompt?.substring(0, 500),
            source_image: opts.imageUrl,
            replicate_url: g.url,
            blob_bytes: bytes,
            persisted: !!blobUrl,
          },
      ...
      results.push({
        ...
        blob: blobUrl,
        bytes,
        id: inserted ? (inserted as { id: number }).id : null,
      });
    }
  } catch (e) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (editedImages.length === 0) {
      ...
      // Log the fallback too — even though Pollinations URLs regenerate per request,
      // we still record the event + prompt for traceability.
      const fallbackResults = await logReimagine({
        brandName: brand,
        imageUrl,
      ...
      const fallbackMap = new Map(fallbackResults.map(r => [r.original, r]));
      const enrichedFallback = editedImages.map(img => ({
        ...img,
        blobUrl: fallbackMap.get(img.url)?.blob || null,
        contentId: fallbackMap.get(img.url)?.id || null,
      }));
      ...
      return NextResponse.json({
        ...
        creativeBrief: briefText,
        generatedImages: enrichedFallback,
        imagePrompt: editDirection,
        productImageUrl,
    ...
    // Persist to Blob + log to brand_content (awaited so UI gets permanent URLs)
    const persistResults = await logReimagine({
      brandName: brand,
      imageUrl,
      generatedImages: editedImages,
      creativeBrief: briefText,
    ...
    // Enrich generated images with blob URLs + brand_content row IDs
    const persistMap = new Map(persistResults.map(r => [r.original, r]));
    const enrichedImages = editedImages.map(img => {
      const p = persistMap.get(img.url);
      return {
        ...img,
        blobUrl: p?.blob || null,                 // permanent copy (preferred for <img>)
        contentId: p?.id || null,                  // brand_content.id of the reimagine row
        bytes: p?.bytes || 0,
      };
    });
    ...
    return NextResponse.json({
      originalAnalysis: imageAnalysis,
      creativeBrief: briefText,
      generatedImages: enrichedImages,
      imagePrompt: editDirection,
      productImageUrl,
      productFrameDescription,
    });
