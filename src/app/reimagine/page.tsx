...
interface GeneratedImage {
  url: string;                 // original source URL (may expire — Replicate 24-48h)
  blobUrl?: string | null;     // permanent Vercel Blob copy
  contentId?: number | null;   // brand_content.id of the reimagine row
  model: string;
}
                ...
                {p.iterations.length > 0 && p.iterations[p.iterations.length - 1].images?.length > 0 && (
                  <div className="grid grid-cols-2 gap-[1px] border-t border-[var(--line)]">
                    {p.iterations[p.iterations.length - 1].images.slice(0, 2).map((img, i) => (
                      <img key={i} src={img.blobUrl || img.url} alt="" className="w-full h-20 object-cover" />
                    ))}
                  </div>
                )}
              </div>
                    ...
                    <div className={`grid ${it.images.length > 1 ? 'grid-cols-2' : 'grid-cols-1'} gap-[1px] bg-[var(--line)]`}>
                      {it.images.map((img, imgIdx) => (
                        <div key={imgIdx} className="bg-[var(--bg)] relative">
                          <img src={img.blobUrl || img.url} alt={`Generated by ${img.model}`} className="w-full aspect-square object-cover" loading="lazy" />
                          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/70 to-transparent px-2 py-1.5 flex items-center justify-between">
                            <span className="text-white text-[10px] font-medium">{img.model}{img.blobUrl && <span className="ml-1 opacity-60">· saved</span>}</span>
                            <a href={img.blobUrl || img.url} target="_blank" rel="noopener noreferrer" className="text-white/80 text-[10px] hover:text-white">Open</a>
                          </div>
                        </div>
                      ))}
                    </div>
