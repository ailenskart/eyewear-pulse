          ...
          <div className="space-y-3">
            ...
            {projects.map(p => (
              <div key={p.id} className="bg-[var(--surface)] border border-[var(--line)] rounded-xl overflow-hidden cursor-pointer" onClick={() => { setActive(p); setView('studio'); }}>
                ...
                {p.iterations.length > 0 && p.iterations[p.iterations.length - 1].image && (
                  <img src={p.iterations[p.iterations.length - 1].image} alt="" className="w-full h-32 object-cover border-t border-[var(--line)]" />
                )}
              </div>
            ))}
          </div>
              ...
              <div key={it.id} className="bg-[var(--surface)] border border-[var(--line)] rounded-xl overflow-hidden" style={{ animation: 'up 0.3s ease' }}>
                ...
                {it.image && (
                  <div className="border-b border-[var(--line)]">
                    <img src={it.image} alt="" className="w-full" />
                    <div className="px-3 py-1.5 flex items-center justify-between bg-[var(--bg-alt)]">
                      <span className="text-[10px] text-[var(--text-3)]">Generated with FLUX AI</span>
                      <a href={it.image} target="_blank" rel="noopener noreferrer" className="text-[11px] text-[var(--brand)] font-semibold">Download</a>
                    </div>
                  </div>
                )}
